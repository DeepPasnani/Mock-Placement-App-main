const config = {
  ...
  auth: {
    ...
    headers: (process.env.AUTH_HEADERS || 'X-Auth-Token').split(' ').filter(Boolean),
  },
  execution: {
    ...
    maxAdditionalFilesSize: parseInt(process.env.MAX_ADDITIONAL_FILES_SIZE) || 2097152,
  ...
  docker: {
    ...
    containerPoolEnabled: process.env.CONTAINER_POOL_ENABLED === 'true',
    maxOutputSize: parseInt(process.env.MAX_OUTPUT_SIZE) || 1048576,
  },
  executor: {
    type: process.env.EXECUTOR_TYPE || 'docker',
  },
  firecracker: {
